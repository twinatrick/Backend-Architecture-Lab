...
public interface UserProjectSkillRepository extends JpaRepository<UserProjectSkill, UUID> {
    ...
    @Modifying
    @Query("delete from UserProjectSkill ups where ups.userId = :userId and ups.project.id = :projectId")
    ...
    @Modifying
    @Query("delete from UserProjectSkill ups where ups.userId = :userId and ups.project.id = :projectId and ups.skill.id = :skillId")
