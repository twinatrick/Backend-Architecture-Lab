...
4.  還原 API 採用 **應用級去重（Atomic Idempotency Claim）**、**樂觀並發防護（JPA @Version Guard）** 與 **Fencing Token 全鏈路** 三重安全守衛：
    ...
    - **破壞性操作前 re-validation**：刪除現存綁定之前再次讀取認領紀錄，確認仍由本次 `ownerId + fencingVersion` 持有；若已被更新的租約接管（例如長時間執行中 lease 過期被 reclaimer 接手），則放棄本次還原，避免舊持有者覆寫新結果。
5.  還原的狀態生命週期以**獨立交易邊界**切分：認領（PROCESSING）以 `REQUIRES_NEW` 先 commit；還原成功以 `REQUIRES_NEW` 的 `markRestoreSuccess` 標記 SUCCESS；還原失敗（含版本衝突與暫態錯誤）以 `REQUIRES_NEW` 的 `markRestoreFailed` 標記 FAILED 並寫入 `lastError` 失敗原因後再對外 rethrow——確保 FAILED/SUCCESS 不因外層交易 rollback 而遺失。`markRestoreSuccess` / `markRestoreFailed` 皆透過 `markRestoreState` 以 `ownerId + fencingVersion` 為 WHERE 條件的 CAS 更新，token 已被更新代數接管則不覆寫（`log.warn`），確保只有最新一代持有者能標記結果。還原本體在 `@Transactional` 中執行：先調用 `userProjectSkillDataAccess.deleteByProjectId(projectId)` 清空該專案的所有現存技能綁定，再依 `beforeState` 記錄逐一 save 重建歷史綁定。
...
- **Fencing Token 防止舊租約覆寫**：`compensation_event_log` 與 `compensation_restore_log` 各持有獨立的 `ownerId + fencingVersion`；接管路徑（`reclaimLease`、`takeOverClaim`）與結果標記（`markRestoreState`）皆以 token 作為 CAS 條件，確保 crash recovery 後由最新一代持有者接手，舊持有者既無法執行破壞性還原也無法覆寫新結果。
