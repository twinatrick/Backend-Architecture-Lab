    ...
    if isinstance(node, dict):
        ...
        if uses_str:
            if has_pr_target and "actions/checkout" in uses_str:
                ...
                if (
                    "github.event.pull_request.head" in ref_val
                    or "github.head_ref" in ref_val
                ):
            ...
            if not uses_str.startswith("./") and not uses_str.startswith("docker://"):
                if "@" in uses_str:
                    ...
                    if not HEX_SHA_REGEX.fullmatch(action_ref.strip()):
                        ...
                        if _is_line_changed(lineno, changed_lines):
                            findings.append(make_finding(
                                ...
                                f"將 Action 鎖定為 40 位元 commit SHA (如 {action_name}@<40-hex-sha>)",
                            ))

        for key_item, val_item in node.items():
