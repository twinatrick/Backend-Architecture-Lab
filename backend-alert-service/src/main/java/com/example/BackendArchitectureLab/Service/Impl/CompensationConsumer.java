 ...
 * <p>
 * 以 event_id 唯一鍵原子領取事件（at-least-once 下同一 eventId 最多執行一次補償），
 * 處理狀態 PROCESSING → PROCESSED / FAILED，FAILED 於下次送達時以 retryClaim CAS 重試，
 * PROCESSING 且租約過期（預設 300 秒）由其他實例以 reclaimLease 接手。
 * 注意：補償執行時間應小於租約時間，否則可能在逾時後被另一實例重複執行——
 * 該重複由 eventId 冪等與 CompensationStrategy 的冪等設計承受（目前策略為 log-only 無副作用）。
 */
@Slf4j
@Service
