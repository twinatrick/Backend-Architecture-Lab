...
class CompensationLeaseReclaimerTest {
    ...
    @Test
    void reclaimExpiredLeases_shouldRetryEligibleFailedEvents() {
        CompensationEventLog failed1 = newLog(CompensationEventLogStatus.FAILED);
        CompensationEventLog failed2 = newLog(CompensationEventLogStatus.FAILED);
        CompensationEventLog retried1 = failed1;
        CompensationEventLog retried2 = failed2;

        when(eventLogRepository.findTop50ByStatusAndNextAttemptAtBeforeOrderByNextAttemptAtAsc(
                eq(CompensationEventLogStatus.FAILED), any(Date.class)))
        ...
        verify(compensationEventProcessor).processReclaimed(retried1);
        verify(compensationEventProcessor).processReclaimed(retried2);
    }

    @Test
    void reclaimExpiredLeases_shouldSkipFailedEvent_whenRetryClaimLostRace() {
        CompensationEventLog failed = newLog(CompensationEventLogStatus.FAILED);
    ...
    @Test
    void reclaimExpiredLeases_shouldContinueWhenFailedRetryProcessingFails() {
        CompensationEventLog failed = newLog(CompensationEventLogStatus.FAILED);
        ...
        doThrow(new IllegalStateException("deserialize failed"))
                .when(compensationEventProcessor).processReclaimed(failed);

        compensationLeaseReclaimer.reclaimExpiredLeases();

        // 重試失敗亦不影響整批；事件會以 FAILED + nextAttemptAt 排入下一輪
    }
}
