...
class CompensationConsumerTest {
    ...
    void setUp() {
        ...
        ReflectionTestUtils.setField(compensationConsumer, "compensationStrategies", List.of(strategy));
        ReflectionTestUtils.setField(compensationConsumer, "leaseSeconds", 300L);
    ...
    @Test
    void handleCompensation_whenCompensationRequiredAndStrategySupports_shouldCompensateAndMarkProcessed() {
        compensationConsumer.handleCompensation(newEvent(CompensationStatus.COMPENSATION_REQUIRED));
    ...
    @Test
    void handleCompensation_whenUnknownAction_shouldThrowAndMarkFailed() {
        ...
        ArgumentCaptor<CompensationEventLog> captor = ArgumentCaptor.forClass(CompensationEventLog.class);
        verify(eventLogRepository).save(captor.capture());
        assertEquals(CompensationEventLogStatus.FAILED, captor.getValue().getStatus());
        assertNotNull(captor.getValue().getLastError());
    }

    @Test
    void handleCompensation_whenCompensated_shouldMarkProcessedWithoutCompensating() {
    ...
    @Test
    void handleCompensation_whenStatusCommitted_shouldMarkProcessedWithoutCompensating() {
    ...
    @Test
    void handleCompensation_whenCompensationFails_shouldMarkFailedAndRethrow() {
    ...
    @Test
    void handleCompensation_recordsEventLogWithProcessingStatusAndLease() {
        ...
        assertTrue(claimed.getLeaseUntil().after(claimed.getProcessingAt()), "租約應在處理時間之後");
        assertEquals(1, claimed.getAttemptCount());
    }

    @Test
    void handleCompensation_whenDuplicateAlreadyProcessed_shouldSkip() {
        stubDuplicate();
    ...
    @Test
    void handleCompensation_whenDuplicateFailedAndRetryClaimSucceeds_shouldProcess() {
        stubDuplicate();
    ...
    @Test
    void handleCompensation_whenDuplicateFailedAndRetryClaimFails_shouldSkip() {
        stubDuplicate();
    ...
    @Test
    void handleCompensation_whenDuplicateProcessingAndLeaseNotExpired_shouldSkip() {
        stubDuplicate();
    ...
    @Test
    void handleCompensation_whenDuplicateProcessingAndLeaseExpired_shouldReclaimAndProcess() {
        stubDuplicate();
    ...
    @Test
    void handleCompensation_whenUnsupportedEventVersion_shouldThrowAndMarkFailed() {
        CompensationEvent event = newEvent(CompensationStatus.COMPENSATION_REQUIRED);
    ...
    @Test
    void handleCompensation_shouldExecuteCompensationOnlyOnce_forDuplicateDelivery() {
        CompensationEvent event = newEvent(CompensationStatus.COMPENSATION_REQUIRED);
