...
- **策略架構**：補償動作透過 `CompensationStrategy` 介面委派（目前為 `ProjectMemberSkillsRebindCompensationStrategy`）；`PROJECT_MEMBER_SKILLS_REBIND` 採取實體「完全擦除並重建」還原模式，呼叫 `/project/inner/skills/restore` 還原 API，並以事件 ID 作為 `Idempotency-Key` 進行應用級去重（寫入 `compensation_restore_log` 去重紀錄）。還原 API 以 `compensation_restore_log` 的 `eventId` 主鍵 + `@Version` 樂觀鎖進行 DB 層級原子認領（INSERT 撞鍵 / UPDATE 樂觀鎖落敗 → 拒絕認領，僅單一實例能執行還原）；認領（PROCESSING）與 SUCCESS / FAILED 狀態切換皆以 `REQUIRES_NEW` 獨立交易 commit，確保失敗標記不因外層回滾而遺失並記錄 `lastError`。並以 snapshot 快照攜帶的專案樂觀鎖 `@Version`（`expectedVersion`）作並發守衛：先快速比對版本，通過後 touch Project 使 `@Version` 於 commit 時執行 CAS 比對，封閉 TOCTOU 窗口，保障補償在並行場景下絕不覆蓋最新合法修改
