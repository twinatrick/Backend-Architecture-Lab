...
- **補償語意**：閉環已建立——業務失敗時 Producer 以 `REQUIRES_NEW` 補發 `COMPENSATION_REQUIRED`（與 `FAILED` 失敗事實並列），Consumer 僅對 `COMPENSATION_REQUIRED` 事件執行補償（成功即為 `COMPENSATED` 語意；失敗標記 `FAILED` 並 rethrow 交由 Kafka 重試 = `COMPENSATION_FAILED`）；`TRANSACTION_STARTED / COMMITTED / FAILED / COMPENSATED` 事件僅記錄後標記完成，不觸發補償；找不到對應 Strategy 的動作視為 `Unsupported compensation action` 拋出例外 → 標記 `FAILED` 交由 Kafka retry（最終 DEAD/DLQ 人工處理），不會被靜默標記完成
...
- **事件狀態機**：`TRANSACTION_STARTED → COMMITTED / FAILED`；補償流程 `FAILED → COMPENSATION_REQUIRED → COMPENSATED / COMPENSATION_FAILED`
