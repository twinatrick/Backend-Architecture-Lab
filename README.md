...
rebindProjectMemberSkills (competency)
  ...
  │   └─ commit
  ├─ 失敗 → rollback 後以 REQUIRES_NEW 寫入 enqueue FAILED + errorMessage
  └─ CompensationOutboxServiceImpl @Scheduled（預設 5 秒）
...
- **可靠性**：`TRANSACTION_STARTED` 與 `COMMITTED` 皆與業務資料**同一交易 commit**（rollback 時一併消失），杜絕「業務成功但 COMMITTED 未建立」的缺口；失敗時在 rollback 後以 `REQUIRES_NEW` 寫入 `FAILED`。Kafka ACK 前不會標記已送達；發送失敗的事件保留於 Outbox 表，以指數退避重試（5s → 15s → 30s → 1m → 5m），超過 `compensation.outbox.max-attempts`（預設 5 次）標記 `DEAD` 供人工查核
...
- **冪等**：傳遞契約為 **At-Least-Once**（非 Exactly-Once）：允許「Kafka ACK → JVM crash → SENT 尚未寫入 DB → 租約過期 → reclaim 重新發送」的 crash window，同一 `eventId` 可能被發送兩次。消費端以 `compensation_event_log`（`event_id` 唯一鍵）原子領取去重：插入成功取得處理權，衝突則依狀態處理（PROCESSED 跳過 / FAILED 以 `retryClaim` CAS 重試 / PROCESSING 且租約未到期跳過），保證同一 `eventId` 最多只產生一次業務副作用；租約過期不代表前一個 Worker 一定失敗（可能仍在執行），重複執行風險一律由 eventId 冪等承受
- **消費端處理狀態機**：`PROCESSING → PROCESSED`；執行失敗 → `FAILED`（記錄 lastError 與 attemptCount）→ 下次同事件送達時以 `retryClaim` 重試；`PROCESSING` 但租約（`compensation.consumer.lease-seconds`，預設 300 秒）已到期 → 其他實例以 `reclaimLease` CAS 接手（crash recovery，不再卡死）
- **消費端重試政策**：由 Kafka `DefaultErrorHandler`（`FixedBackOff(1000ms, 4)`，共 5 次嘗試）負責重試，不另建第二套排程器；重試耗盡後無 DLT，由 error handler 記錄錯誤並停止該事件
- **監控告警**：`CompensationMonitor`（alert）定期（`compensation.monitor.interval-ms`，預設 60 秒）掃描滯留 `FAILED` 的消費事件並 `log.error`（含 eventId / attemptCount / lastError）；`CompensationOutboxMonitor`（competency）同週期掃描重試耗盡的 `DEAD` Outbox 事件並 `log.error`，供人工介入；「執行時間超過租約（300 秒）導致被其他實例 reclaim 重複執行」屬預期行為，若監控告警頻繁出現重複處理代表補償執行過慢，需檢討策略設計
- **補償語意**：Consumer 僅對 `COMPENSATION_REQUIRED` 事件執行補償（成功即為 `COMPENSATED` 語意；失敗標記 `FAILED` 並 rethrow 交由 Kafka 重試 = `COMPENSATION_FAILED`）；`TRANSACTION_STARTED / COMMITTED / FAILED / COMPENSATED` 事件僅記錄後標記完成，不觸發補償；找不到對應 Strategy 的動作視為 `Unsupported compensation action` 拋出例外 → 標記 `FAILED` 交由 Kafka retry（最終 DEAD/DLQ 人工處理），不會被靜默標記完成
- **策略架構**：補償動作透過 `CompensationStrategy` 介面委派（目前僅 `ProjectMemberSkillsRebindCompensationStrategy`）；`PROJECT_MEMBER_SKILLS_REBIND` 目前為 log-only（無不可逆 side effect，可承受重複 delivery），依賴本地 DB transaction rollback 保證一致性；未來若改呼叫外部服務（Feign/REST/DB 寫入），**必須以 `eventId` 作為冪等鍵**（如 `Idempotency-Key` header），避免重複 delivery 造成重複副作用
- **版本化**：`eventVersion` 標記事件 schema 版本（目前 1）；Consumer 拒絕不支援的版本（`eventVersion != 1` 拋出 `UnsupportedEventVersionException`）→ 標記 `FAILED` → Kafka retry，不會誤當版本 1 處理
- **事件狀態機**：`TRANSACTION_STARTED → COMMITTED / FAILED`；補償流程 `COMPENSATION_REQUIRED → COMPENSATED / COMPENSATION_FAILED`
- **發佈邊界**：業務 Service 僅依賴 `ICompensationOutboxService`（寫入 Outbox），Kafka 發佈只由 Outbox Worker（`CompensationOutboxServiceImpl`）透過 `ICompensationPublisher`（僅 `publish` 單一方法，事件由 Worker 自行組裝）執行，兩層介面隔離
