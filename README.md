...
- **冪等**：每個事件帶唯一 `eventId`，消費端以 `compensation_event_log`（`event_id` 唯一鍵）原子領取去重：插入成功取得處理權，衝突則依狀態處理（PROCESSED 跳過 / FAILED 以 `retryClaim` CAS 重試 / PROCESSING 且租約未到期跳過），at-least-once 語意下不重複執行
...
- **策略架構**：補償動作透過 `CompensationStrategy` 介面委派（目前僅 `ProjectMemberSkillsRebindCompensationStrategy`）；`PROJECT_MEMBER_SKILLS_REBIND` 目前依賴本地 DB transaction rollback 保證一致性，Strategy 僅為未來跨服務補償（呼叫其他服務進行 rollback/回復）的擴充點
